  ...
  <div>
    ...
    <Modal v-model="showEdit" title="修改提现设置" width="480">
      <Form :model="editRow" ref="editFormValidate" label-position="right" :label-width="140">
        ...
        <FormItem label="最小兑换" prop="MinExchange" :rules="{ required: true, message: '请输入房间排序！', trigger: 'blur' }">
          <Input v-model="editRow.MinExchange" style="width:200px"></Input>
        </FormItem>
        <FormItem label="最大兑换" prop="MaxExchange" :rules="{ required: true, message: '请填写税率！', trigger: 'blur' }">
          <Input v-model="editRow.MaxExchange" style="width:200px"></Input>
        </FormItem>
        <FormItem label="税收" prop="Tax" :rules="{ required: true, message: '请输入底分！', trigger: 'blur' }">
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const res = this.handleResult(result, ['pkid', 'MinExchange', 'MaxExchange', 'Tax', 'State'])
