          ...
          if (data.arrDatas && data.arrDatas.length) {
            for (let i = 0; i < data.arrDatas.length; i++) {
              this.tableDatas.push(
                {
                  pkid: data.arrDatas[i].arrString[0],
                  MsgTitle: data.arrDatas[i].arrString[1],
                  MsgContent: data.arrDatas[i].arrString[2],
                  StartTime: data.arrDatas[i].arrString[3],
                  EndTime: data.arrDatas[i].arrString[4]
                }
              )
            }
          }

          this.total = data.nTotal ? data.nTotal : 0
        }
      } catch (error) {
      }
