...
const user = {
  ...
  actions: {
    initWebSocket: ({commit, state}) => {
      const wsuri = 'ws://tx.efzhe.com/wsapp' // ws地址 uu
      // const wsuri = 'ws://www.servicechat.local/wsapp' // ws地址 uu
      ...
      state.websocket.onmessage = (e) => { // 数据接收
        ...
        switch (type) {
          ...
          default:
            ...
            document.getElementById('toQuery').click()
            Notice.open({
              title: '用户' + data.user_id + '申请提现',
              name: name,
              duration: 300,
              render: h => {
                return h('div', [
                  ...
                  on: {
                    click: () => {
                      const result = api.updateStatus({ status: 1, id: data.id })
                      if (result) {
                        if (!result.status) {
