    ...
    <div>
        <Form label-position="right" :label-width="70" inline>
            ...
            <FormItem :label-width="1">
                <Button type="primary" @click="toQuery" :loading="queryLoading" icon="ios-search" id="toQuery">查询</Button>
            </FormItem>
        ...
        <Modal v-model="showEdit" :title="'银行卡信息'" width="350" :class-name="chatModalClass" @on-cancel="cancel" id="Modal">

            <div style="line-height: 30px;font-size: 14px;">
                <span style="float: right;"><Button type="primary" class="tag-read" v-bind:data-clipboard-text="name" @click="copy" size="small">复制</Button></span>
                <span style="margin-right:10px;">姓名:</span>{{name}}
            </div>
            <div style="line-height: 30px;font-size: 14px;">
                <span style="float: right;"><Button type="primary" class="tag-read" v-bind:data-clipboard-text="bank_code" @click="copy" size="small">复制</Button></span>
                <span style="margin-right:10px;">银行:</span>{{bank_code}}</div>
            <div style="line-height: 30px;font-size: 14px;">
                <span style="float: right;"><Button type="primary" class="tag-read" v-bind:data-clipboard-text="bank_card" @click="copy" size="small">复制</Button></span>
                <span style="margin-right:10px;">卡号:</span>{{bank_card}}</div>
            <div style="line-height: 30px;font-size: 14px;">
                <span style="float: right;"><Button type="primary" class="tag-read" v-bind:data-clipboard-text="bill_price" @click="copy" size="small">复制</Button></span>
                <span style="margin-right:10px;">金额:</span>{{bill_price}}</div>
            <div slot="footer" class="send-box">
                <div style="margin: auto">
                    <Button type="primary" @click="updateStatus(3)" :loading="loading">完成</Button>
                    <Button type="error"   @click="updateStatus(2)" :loading="loading">拒绝</Button>
...
<script>
    ...
    export default {
                                        ...
                                        on: {
                                            click: () => {
                                                if (this.isPunch) {
                                                    ...
                                                    this.bank_code = params.row.bank_code
                                                    this.bank_card = params.row.bank_card
                                                    this.bill_price = params.row.bill_price
                                                    if (params.row.status == 0) //待处理
                                                    {
                                                        this.hold ()    //更新状态为已受理
                                                    }
                                                    else if (params.row.status == 1) //已受理
                                                    {
                                                        this.showEdit = true
                                                    }
        ...
        computed: {
            ...mapGetters(['isPunch', 'isPunchText', 'chatModalClass'])
        },
        methods: {
            init () {
                this.queryList()
            ...
            async hold () {
                try {
                    const result = await this.$store.dispatch('updateStatus', { status: 1, id: this.id })
                    if (result) {
                        if(!result.status){
            ...
            async updateStatus (status) {
                try {
                    const result = await this.$store.dispatch('updateStatus', { status: status, id: this.id })
                    if (result) {
                        if(!result.status){
                            this.$Message.warning(result.msg)
                            setTimeout(() => {
