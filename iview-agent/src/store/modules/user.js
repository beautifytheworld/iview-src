...
import { constantRouterMap, asyncRouterMap, asyncRouterMap2, errorRouterMap } from '@/router/routes'
...
const user = {
  ...
  actions: {
    ...
    getUserMenu: ({ commit, state }) => {
      return new Promise((resolve, reject) => {
        // 暂时无权限控制 分配所有菜单路由
        let routes = getKey('nLv') >= 4 ? asyncRouterMap : asyncRouterMap2
    ...
    agInsertGame: ({commit}, data) => {
      return api.agInsertGame(data)
    },
    getGameCustomSetting: ({commit}, data) => {
      return api.getGameCustomSetting(data)
    },
    getGameInfo: ({commit}, data) => {
      return api.getGameInfo(data)
    },
    agDeleteGame: ({commit}, data) => {
      return api.agDeleteGame(data)
    },
