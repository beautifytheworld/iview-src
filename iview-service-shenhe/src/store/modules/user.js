      ...
      return new Promise((resolve, reject) => {
        ...
        setInterval(async () => {
          try {
            var query = {
              Status: 0,
              start_time: moment().subtract(2, 'minutes').format('YYYY-MM-DD HH:mm:ss')
              // end_time: moment().format('YYYY-MM-DD HH:mm:ss')
            }
            var res = await api.getExchangeOrders(query)
            if (res && res.data && res.data.nTotal) {
              ...
              Notice.warning({
                ...
                render: h => {
                  return h('div', {
                    ...
                    on: {
                      click: () => {
                        router.push({ name: 'exchange_orders' })
                        Notice.close(name)
                      }
                    } }, '去查看')
                  ])
