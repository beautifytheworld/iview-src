  ...
  <div>
    <Form label-position="right"
      ...
      <FormItem label="商户名:" class="">
        <Input clearable type="text" v-model="query.MerchantName" placeholder="填写商户名"></Input>
      </FormItem>
...
export default {
  ...
  data () {
    return {
      ...
      loading: false,
      query: { userid: '' },
      statusObj: { '0': '待处理', '1': '完成', '2': '失败', '3': '处理中' },
      callbackStatusObj: { '0': '未回调', '1': '回调完成', '2': '回调失败' },
      datasTime: [],
      page: 1,
      per_page: 20,
      total: 0,
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const result = await this.$store.dispatch('getApiRecordList', data)
        ...
        for (let i = 0; i < columnKeys.length; i++) {
          ...
          if (obj.key === 'CallbackStatus') {
            obj.render = (h, params) => {
              return h('span', this.callbackStatusObj[params.row.CallbackStatus])
            }
          }
